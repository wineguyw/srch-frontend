...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Dynamically populate country filter with all countries from wineRegions
  function populateCountryFilter() {
    // Clear existing options except the placeholder
    while (countryFilter.options.length > 1) {
      countryFilter.remove(1);
    }
    // Add new options from wineRegions
    Object.keys(wineRegions).forEach(country => {
  ...
  populateCountryFilter(); // Call this on page load to populate countries
  ...
  console.log("✅ MapBox Globe Initialized with Dynamic Country Filter and Expanded Wine Regions!");
});