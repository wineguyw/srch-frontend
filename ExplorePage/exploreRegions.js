...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Wine region data (simplified for clarity, matching your original)
  const wineRegions = {
    USA: { lat: 39.8283, lng: -98.5795, zoom: 2, regions: {
      "Napa Valley": { lat: 38.2975, lng: -122.2869, zoom: 8, subregions: [
      ...
      "Sonoma County": { lat: 38.433, lng: -122.515, zoom: 8, subregions: [
  ...
  const map = new mapboxgl.Map({
    container: "globeViz",
    style: "mapbox://styles/mapbox/satellite-v10", // High-resolution satellite imagery for clarity
    center: [0, 20], // Centered on a global view (adjust as needed)
    zoom: 1, // Start at a global zoom level
    projection: "globe", // Use 3D globe projection for clear, spherical rendering
    maxZoom: 22, // Allow high zoom for subregions (MapBox supports up to 22)
  ...
  // Disable scroll zoom initially if needed, enable on interaction
  ...
  // Add atmosphere for a realistic globe (optional, matches your dark theme)
  map.on("style.load", () => {
    map.setFog({
      color: "rgba(158, 141, 141, 0.3)", // Matches your atmosphere color
      "high-color": "#add8e6",
      "horizon-blend": 0.02,
      "star-intensity": 0.6
    });
  ...
  // Generate rings for wine regions (simulating your concentric rings)
  const generateRings = (location) => {
    const rings = [];
    const numRings = 5; // Reduced for clarity at high zooms
    const baseSize = 0.1; // Smaller for MapBox scale
    const spacing = 0.05;
    ...
    for (let i = 0; i < numRings; i++) {
      rings.push({
        ...
        properties: {
          radius: baseSize + i * spacing,
          speed: 0.05 + i * 0.01,
          period: 4000 + i * 500,
          color: `rgba(255, 69, 0, ${0.7 - i * 0.1})`
        }
      });
    }
    return rings;
  ...
  map.on("load", () => {
    ...
    // Animate rings (simulate propagation)
  ...
  let currentZoomLevel = 1.0; // Start at global zoom
  ...
  function stopAndZoom(lat, lng, zoom) {
    ...
    map.easeTo({
      ...
      zoom: zoom,
      duration: 1500,
      pitch: 0 // Optional: Keep flat for consistency
    });
  }

  // Country Selection - Zoom In
  ...
  subregionFilter.addEventListener("change", () => {
    ...
    if (subregion) {
      ...
      stopAndZoom(subregionData.lat, subregionData.lng, 12); // Higher zoom for subregions (up to 22 max, but 12 is clear for subregions)
    }
  });

  // Add labels for wine regions (optional, for clarity)
  map.on("load", () => {
  ...
  console.log("✅ MapBox Globe Initialized with Auto-Rotation and Clear Zoom!");
});